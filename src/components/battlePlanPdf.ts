  ...
  if (visibilityScore) {
    ...
    CATEGORY_ORDER.forEach((key: CategoryKey) => {
      ...
      const inputLine = cat?.provenance === "missing"
        ...
        : (CATEGORY_INPUT_FORMATTERS[key](visibilityScore.rawMetrics) ?? "Raw inputs weren't recorded for this run.");
...
// formatPerformanceInputs / formatSocialInputs / formatThoughtLeadershipInputs /
// formatReputationInputs live in src/lib/visibilityInputFormatters.ts —
// shared with the live Dashboard's category explainer so the PDF and the
// app never show two independently-written summaries of the same evidence.
